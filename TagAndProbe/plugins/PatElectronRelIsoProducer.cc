...
/*
  \class    	  PatElectronRelIsoProducer
  \description:
...
class PatElectronRelIsoProducer : public edm::EDProducer {
 public:
  explicit PatElectronRelIsoProducer(const edm::ParameterSet & iConfig);
  virtual ~PatElectronRelIsoProducer() ;
 ...
 private:
  edm::InputTag inputTagPatElectrons_;
...
PatElectronRelIsoProducer::PatElectronRelIsoProducer(const edm::ParameterSet & iConfig) :
  inputTagPatElectrons_(iConfig.getParameter<edm::InputTag>("ElectronProbes")),
...
PatElectronRelIsoProducer::~PatElectronRelIsoProducer()
{
}

void PatElectronRelIsoProducer::produce(edm::Event & iEvent, const edm::EventSetup & iSetup) {
  
  // Electron Collection (from Reco)
  edm::Handle<pat::ElectronCollection> h_Electron;
  bool found = iEvent.getByLabel(inputTagPatElectrons_,h_Electron);
  if(!found ) {
    std::ostringstream  err;
    err<<" cannot get PatElectrons: "
       <<inputTagPatElectrons_<<std::endl;
    edm::LogError("PatElectronRelIsoProducer")<<err.str();
  ...
  // Loop on Electrons
  unsigned nele=h_Electron->size();
  for(unsigned iele=0; iele<nele; ++iele) {
    pat::ElectronRef ele(h_Electron, iele);
    // Get Original Reco Electron
  ...
  fill_reliso.insert(h_Electron, v_reliso.begin(), v_reliso.end());
  fill_reliso.fill();
...
DEFINE_FWK_MODULE(PatElectronRelIsoProducer);
